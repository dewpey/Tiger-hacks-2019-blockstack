  ...
  render() {
    ...
    return ( <
        ...
        style = {
          ...
          src = {
            `${app}/animals/${rulerAnimal.id}.jpg`
          }
          alt = {
            rulerAnimal.name
          }
        ...
        <
        p > < a href = {
          `${EXPLORER_URL}/name/${username}`
        }
        target = "_blank"
        rel = "noopener noreferrer" > {
          username
        } < /a> is a {rulerAnimal.name} that rules over the {rulerTerritory.name}.</p >
        :
        <
        p > {
          username
        }
        is an unknown animal that hails from an unknown land. < /p>
      } <
      p > {
        mine ? < Link className = "btn btn-primary"
        to = "/me"
        role = "button" > Edit my animal < /Link> : <
        a
        className = 'btn btn-primary'
        href = {
          `${window.location.origin}/kingdom/${currentUsername}?add=${app}/kingdom/${username}`
        } >
